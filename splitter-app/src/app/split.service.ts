      ...
      if(personsWithDept.length==0){
       ...
       while (indexCreditor<personsWithCredit.length
           ...
           if (Math.abs(debtor.credit)==Math.abs(creditor.credit)){
            let transaction:Transaction = {debtor:debtor, creditor:creditor,amount:Math.round(Math.abs(debtor.credit) * 100) / 100};
           ...
           //then we go to the next creditor (increment the creditors index only)
             ...
             let transaction:Transaction = {debtor:debtor, creditor:creditor,amount:Math.round(Math.abs(creditor.credit) * 100) / 100};
             result.push(transaction);
